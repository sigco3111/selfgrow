...
**현재 단계**: Phase 0 (코어 엔진) ✅ / Phase 1 (밸런싱+전투) ✅ / Phase 2 (전투 심화+파벌) ✅ / Phase 3 (SmartBrain) 진행 중
...
- **Phase 3** (진행 중): SmartBrain — 경험 기반 학습, 멀티스텝 계획, 목표 설정, 개체 간 메시징 (LLM 없음, 순수 로컬 알고리즘)
- **Phase 4** (예정): 데이터 익스포트(CSV/JSON), 외부 분석 도구 연동
